...
def run_validate_cli():
    def _run(manifest_path: Path, schema_path: Path = SCHEMA) -> tuple[int, str, str]:
        return run(
            [sys.executable, str(VALIDATOR), "validate", str(manifest_path), str(schema_path)]
        )
...
def tmp_page_factory(tmp_path):
    def _make_page(
        namespace: str = "Template", name: str | None = None, content: str = "== Page ==\n"
    ) -> dict:
        if name is None:
    ...
    mpath = manifest(
        {
            ...
            "packs": {"example": {"version": "1.0.0", "pages": ["Person"]}},
        }
    )
    rc, out, err = run_validate(mpath)
    assert rc == 0
...
def test_validate_manifest_without_extra_sections_is_ok(manifest, run_validate, tmp_path):
    mpath = manifest({"pages": {}, "packs": {"a": {"version": "1.0.0", "pages": []}}})
    rc, out, err = run_validate(mpath)
    assert rc == 0
...
def test_validate_valid_page_last_updated_passes(
    manifest, run_validate, tmp_path, tmp_page_factory
):
    page = tmp_page_factory(name="Example")
    page.pop("version", None)
    ...
    mpath = manifest(
        {
            "pages": {"Template:Example": page},
            "packs": {
                "example": {"version": "1.0.0", "pages": ["Template:Example"], "depends_on": []}
            },
        }
    )
    rc, out, err = run_validate(mpath)
...
def test_validate_duplicate_page_across_packs_fails(
    manifest, run_validate, tmp_path, tmp_page_factory
):
...
def test_pack_pages_or_two_deps(
    manifest, run_validate, tmp_page_factory, case_id, packs_builder, expect_ok
):
...
def test_schema_auto_falls_back_when_major_unmapped(
    tmp_path, run_validate, tmp_page_factory, manifest
):
...
def test_pack_tags_variants(
    tmp_path, run_validate, tmp_page_factory, manifest, tags, expect_ok, expect_msg_any
):
    page = tmp_page_factory(name="Example")
    mpath = manifest(
        {
...
def test_schema_override_via_dollar_schema_field(
    tmp_path, run_validate, manifest, tmp_page_factory
):
...
def test_schema_version_requires_strict_semver_in_auto(
    tmp_path, run_validate, manifest, tmp_page_factory
):
    page = tmp_page_factory(name="X")
    mpath = manifest(
...
def test_rejects_colon_in_filename(manifest, run_validate, tmp_path):
    ...
    mpath = manifest(
        {
            "pages": {
                "Template:Bad": {"file": "pages/Templates/Template:Bad.wiki", "version": "1.0.0"}
            },
...
def test_warns_on_title_missing_namespace(manifest, run_validate, tmp_page_factory):
    page = tmp_page_factory(namespace="")
    mpath = manifest(
        {"pages": {"Person": page}, "packs": {"ex": {"version": "1.0.0", "pages": ["Person"]}}}
    )
    rc, out, err = run_validate(mpath)
    assert rc == 0
